  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white border border-slate-200 rounded-lg shadow-lg p-3">
        <p className="font-semibold text-slate-900 mb-1">{data.day}</p>
...
export default function DayOfWeekChart({ data, title = 'Viewing by Day of Week', isLoading = false }) {
  // Ensure all days are present in the data
  const completeData = DAYS_OF_WEEK.map((day, index) => {
    const existing = data.find(d => d.dayOfWeek === index)
    return existing || { day: day, dayOfWeek: index, count: 0 }
  })
